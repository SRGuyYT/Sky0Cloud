...
### Key Features
* CSP-compliant: JavaScript is externalized to `welcome.js` to satisfy strict Content Security Policies and prevent inline script blocking.
...
* Token-gated sign-up: Homeserver registration is enabled only with the registration token `only_us`.
